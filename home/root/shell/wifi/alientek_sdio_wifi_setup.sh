...
#正点原子@ALIENTEK 
#SDIO WIFI脚本 
#功能:脚本支持station模式、softap模式、bridge模式相互切换（若相互切换不成功，请重复执行）
...
if [ "$mode" == "bridge" ]; then 
	...
	#	wpa_supplicant -B -Dnl80211 -i $device -c /etc/wpa_supplicant.conf
	#fi
	#sleep 2
	#processkill
	#sleep 1
...
update_config=1\nnetwork={\nssid=\"$ssid\"\npsk=\"$psk\"\n}\n" > $wifi_conf
	...
	ifconfig wlan0 up
	sleep 2
	wpa_supplicant -B -Dnl80211 -i $device -c $wifi_conf
	udhcpc -R -b -i wlan0
